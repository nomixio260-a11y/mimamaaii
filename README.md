...
| ニューラル LM | numpy があれば、デコーダ専用 Transformer (既定 d=128・4 ヘッド・2 層・文脈 64、語彙は学習データから自動) を知識文と会話ペア (`<usr>` 発話 `<bot>` 応答) で継続学習。取り置き文の ppl が n-gram の 1.5 倍以内に入ると生成候補に加わり、それまでは候補の採点 (自然さ) にだけ使う。自律ループの各サイクルで数秒ずつ学習し、5 分ごとに保存 |
...
python -m tinyai train --steps 2000 --seconds 600
...
### ニューラル LM の実測 (numpy 2.4、CPU 4 コア)

| 項目 | 値 |
|---|---|
| パラメータ | 657k (語彙 1,969 の場合) 〜 0.93M (語彙 4,096) |
| 学習速度 | 約 10,000 トークン/秒 |
| 損失 / 取り置き ppl (日本語 Wikipedia 3 記事、400 ステップ 80 秒) | 8.34 → 3.79 / 996 → 75 |
| 同じデータの KN 3-gram の ppl | 27 |

数万トークンでは n-gram に届きません (ニューラル LM は桁違いのデータで n-gram を超えます)。
本体は「n-gram + 検索 + 事実」を主とし、ニューラル LM は候補の自然さの採点から始めて、
学習が進み ppl が n-gram の 1.5 倍以内に入った時点で自動的に生成候補に加わる設計です。
青空文庫・公開データセット・ダンプで数千万トークンを流せば、CPU 1 台で数時間の学習になります。
