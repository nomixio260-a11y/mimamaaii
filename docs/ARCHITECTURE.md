...
tinyai/
  ...
  lm.py           n-gram 言語モデル (整数パック文脈、KN 継続カウント、剪定、生成)
...
発話 ─ コマンド (覚えて/調べて/👍/👎/もっと詳しく)
     ...
     ─ kb.search (+ 低確信なら PMI 関連語で拡張) → 質問タイプ別リランク → recall / guess
     ─ lm.generate (見つかった知識の語を優先) → generate
...
| 予算 | (上限×0.85 − 基礎 RSS) × 0.8^tighten。LM 55% / KB 35% |
