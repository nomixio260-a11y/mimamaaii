...
`train --serve PORT` は学習ループと同じプロセスで HTTP サーバーをデーモンスレッドとして開く。
学習スレッドと会話スレッドが同じ重みを触るため、`NeuralLM` は再入可能ロックを持ち、
学習・生成・評価・EMA への切替をすべてその中で行う (EMA に切り替えている最中に AdamW が走ると
平均重みの側を更新してしまうため)。`/ask` は応答に加えて思考過程 (下書き・再検索・候補の採点) と
ニューラル LM の統計を返し、`/stats` は学習の進み (step, loss, tok/s, ppl, 収集した文数) を返す。
...
| 会話としての予測力 | 応答部だけの ppl (`dialog_ppl`)。文の ppl と分けて測る |
| 復号バイアスの掃引 (`--sweep`) | 文脈への寄せ方を 0〜5 で振り、接地率と自然さの釣り合いを見る |
| 退行検出 (`--compare` / `--fail-on-regress`) | 前回より 5% 以上悪化した指標に ⚠ を付け、CI では終了コード 1 |
| 学習中の自己評価 | `train --eval-every 秒` で Brain.self_evaluate() を定期実行し `eval {...}` 行に出す。export が meta.json に取り込み、ブラウザ版が品質曲線として描く |
