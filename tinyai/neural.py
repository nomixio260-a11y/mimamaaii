...
# base は 9 → 12 に引き上げた後、実測を見て 11 に戻した。9 → 11 層 (710 万 → 1,468 万パラメータ) で
# 学習速度は 4,700 → 2,400 トークン/秒に落ち、平文の品質は改善していない (10 層の判定は
# 平文 2.958 → 3.602 bit/字、会話 54.5 → 53.4)。**増やすより使いこなす**段階なので、
# 今の大きさで consolidate させる。データがさらに増えて頭打ちが見えたら、また上げればよい。
MAX_LAYERS = {"small": 5, "base": 11, "large": 14, "xl": 16}
