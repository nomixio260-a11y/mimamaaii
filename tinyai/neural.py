...
# base を 9 → 12 に引き上げた: 中間次元は上限 (ff 1562) に達し、手持ちのデータは 2 億 9,000 万トークン
# (パラメータ 1,190 万に対し 24 トークン/パラメータ) あるので、まだ容量を増やす余地がある。
# 深くすると崩れやすいが、今は (a) 深さと幅に応じた学習率の引き下げ、(b) 2000 ステップの馴染ませ期間、
# (c) 成長前の品質を保存した取り消し、(d) 成長の前後の記録、が揃っているので、悪化すれば戻せる。
MAX_LAYERS = {"small": 5, "base": 12, "large": 14, "xl": 16}
